...
if ( ! defined( 'EMOZA_VERSION' ) ) {
	...
	define( 'EMOZA_VERSION', '1.1.0' );
}

// Emoza White Label Compatibility
if( function_exists( 'emoza_wl_get_data' ) ) {
	$emoza_awl_data = emoza_wl_get_data();

	if( isset( $emoza_awl_data[ 'activate_white_label' ] ) && $emoza_awl_data[ 'activate_white_label' ] ) {
		define( 'EMOZA_AWL_ACTIVE', true );
	}
}
...
function emoza_register_required_plugins() {
    ...
    $plugins = array(
        array(
            ...
            //'external_url'       => 'https://yourserver.com/plugins/kirki.zip', // Fallback URL if the plugin is not found in the WordPress Plugin Directory.
        ),
...
if( defined( 'EMOZA_PRO_VERSION' ) ) {
	if( version_compare( EMOZA_PRO_VERSION, '1.1.0', '>=' ) ) {
			...
			<div class="notice notice-warning" style="position:relative;">
				<p>
					...
					printf(
						...
						esc_html__(
							'It looks like your website is running Emoza Pro but not with its latest version. Please note that Emoza 1.1.9+ (free theme) requires Emoza Pro updated to a minimum version of 1.1.0. For it please go to %s and update Emoza Pro.', 'emoza-woocommerce'
						),
						'<a href="'. esc_url( admin_url( 'plugins.php' ) ) .'">' . esc_html__( 'Plugins', 'emoza-woocommerce' ) . '</a>'
					);
					?>
				</p>
			</div>
