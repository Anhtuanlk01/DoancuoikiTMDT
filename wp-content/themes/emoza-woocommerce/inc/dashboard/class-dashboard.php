...
class Emoza_Dashboard
{
    ...
    public function __construct() {
        ...
        if( defined( 'EMOZA_PRO_VERSION' ) && ! $is_legacy_tb ) {
            add_action( 'wp_ajax_emoza_template_builder_data', array( $this, 'ajax_template_builder_data' ) );
    ...
    public function add_menu_page() {
        ...
        add_submenu_page( // phpcs:ignore WPThemeReview.PluginTerritory.NoAddAdminPages.add_menu_pages_add_submenu_page
            'emoza-dashboard',
            esc_html__('Templates Builder', 'emoza-woocommerce'),
            esc_html__('Templates Builder', 'emoza-woocommerce'),
            'manage_options',
            get_admin_url() . 'admin.php?page=emoza-dashboard&tab=builder',
            '',
            3
        );
        ...
        if( ! defined( 'EMOZA_PRO_VERSION' ) ) {
            add_submenu_page( // phpcs:ignore WPThemeReview.PluginTerritory.NoAddAdminPages.add_menu_pages_add_submenu_page
                'emoza-dashboard',
                esc_html__('Upgrade to Pro', 'emoza-woocommerce'),
                esc_html__('Upgrade to Pro', 'emoza-woocommerce'),
                'manage_options',
                'https://emoza.org?utm_source=theme_submenu_page&utm_medium=button&utm_campaign=Emoza',
                '',
                5
            );
        }
    ...
    public function ajax_template_builder_data() {
        ...
        update_option('emoza_template_builder_data', $data);
    ...
    public function insert_template_part_callback() {
		...
		if ( empty( $post ) ) {
			...
			$post_title     = __( 'Emoza Template Part - ', 'emoza-woocommerce' ) . str_replace( 'emoza-template-', '', $key ) . '-' . sanitize_text_field( wp_unslash( $_POST['part_type'] ) );
    ...
    public function existing_parts_select( $parts = array() ) {
        ...
        if ( empty( $parts ) ) {
            $html .= '<div>' . esc_html__( 'No templates found.', 'emoza-woocommerce' ) . '</div>';
        } else {
            $html .= '<select class="existing-parts-select">';
            $html .= '<option value="">' . esc_html__( 'Select existing', 'emoza-woocommerce' ) . '</option>';
