...
function emoza_dashboard_settings() {
	...
	// Documentation.
	//
	$settings['documentation_link'] = 'https://emoza.org/docs/';
	...
	$settings['suggest_idea_link'] = 'https://emoza.org/';

	//
	// Knowledge Base.
	//
	$settings['knowledge_base_link'] = 'https://emoza.org/docs/';

	//
	// Support.
	//
	$settings['support_link']     = 'https://emoza.org/support/';
	$settings['support_pro_link'] = 'https://emoza.org?utm_source=theme_support&utm_medium=button&utm_campaign=Emoza';
	...
	if ( ! $is_legacy_tb && ( isset( $settings['has_pro'] ) && $settings['has_pro'] && Emoza_Modules::is_module_active( 'templates' ) ) || !$is_legacy_tb && ! $settings['has_pro'] ) {
	...
	if ( class_exists( 'Merchant' ) && defined( 'MERCHANT_VERSION' ) && version_compare( MERCHANT_VERSION, '1.9.2', '>' ) ) {
		$settings['settings']['merchant'] = esc_html__('Merchant', 'emoza-woocommerce');
	}
	...
	$settings['features'][] = array(
		...
		'docs_link' => 'https://emoza.org/docs/',
	);
...
add_filter( 'emoza_register_demos_settings', 'emoza_demos_settings' );
