...
/**
 * Add emoza class to nav menu ul sub-menu.
 *
 ...
 */
function emoza_nav_menu_submenu_css_class( $classes, $args, $depth ) {
	$classes[] = 'emoza-dropdown-ul';
	return $classes;
}
add_filter( 'nav_menu_submenu_css_class', 'emoza_nav_menu_submenu_css_class', 10, 3 );

/**
 * Add emoza class to nav menu li.
 * 
 ...
 */
function emoza_nav_menu_css_class( $classes, $menu_item, $args, $depth ) {
	$classes[] = 'emoza-dropdown-li';
	return $classes;
}
add_filter( 'nav_menu_css_class', 'emoza_nav_menu_css_class', 10, 4 );

/**
 * Add emoza class to nav menu anchor.
 * 
 ...
 */
function emoza_nav_menu_link_attributes( $atts, $menu_item, $args, $depth ) {
	$atts[ 'class' ] = 'emoza-dropdown-link';
	return $atts;
}
add_filter( 'nav_menu_link_attributes', 'emoza_nav_menu_link_attributes', 10, 4 );
...
function emoza_body_classes( $classes ) {
	...
	$classes[] = 'emoza-site-layout-' . get_theme_mod( 'site_layout', 'default' );
 ...
 */
function emoza_get_svg_icon( $icon, $do_echo = false, $wpkses = true ) {
	...
	/**
	 * Hook 'emoza_get_svg_icon_output'.
	 ...
	 * @since 1.1.0
	 */
	$svg_code = apply_filters( 'emoza_get_svg_icon_output', $svg_code, $icon );
...
add_action( 'wp', 'emoza_page_options' );
...
function emoza_social_profile( $location ) {
	...
	foreach ( $social_links as $social ) {
		...
		if ( $network ) {
			...
			$items .= '<a target="_blank" href="' . esc_url( $social ) . '" title="' . sprintf( esc_attr__( 'Social network %s link.', 'emoza-woocommerce' ), $network ) . '"><i class="ws-svg-icon">' . emoza_get_svg_icon( 'icon-' . esc_html( $network ), false ) . '</i></a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
	$items .= '</div>';
...
function emoza_header_builder_components() {
	...
	/**
	 * Hook 'emoza_header_builder_components'
	 *
	 * @since 1.0.0
	 */
...
function emoza_mobile_header_builder_components() {
	...
	/**
	 * Hook 'emoza_mobile_header_builder_components'
	 *
	 * @since 1.0.0
	 */
...
function emoza_page_title() {
	if ( is_home() && ! is_front_page() && ! get_theme_mod( 'archive_hide_title', 0 ) ) :
		...
		<header class="page-header">
			<h1 class="page-title" <?php emoza_schema( 'headline' ); ?>><?php single_post_title(); ?></h1>
		</header>
		<?php
	endif;
}
...
function emoza_get_default_header_components() {
	...
	/**
	 * Hook 'emoza_default_header_components'
	 *
	 * @since 1.0.0
	 */
...
function emoza_header_layouts() {
	...
	/**
	 * Hook 'emoza_header_layout_choices'
	 *
	 * @since 1.0.0
	 */
...
function emoza_mobile_header_layouts() {
	...
	/**
	 * Hook 'emoza_mobile_header_layout_choices'
	 *
	 * @since 1.0.0
	 */
...
function emoza_header_elements_layout_7_8() {
	...
	$elements = array(
		...
		'button'            => esc_html__( 'Button', 'emoza-woocommerce' ),
		'contact_info'      => esc_html__( 'Contact info', 'emoza-woocommerce' ),
		'hamburger_btn'     => esc_html__( 'Hamburger button', 'emoza-woocommerce' ),
	);

	/**
	 * Hook 'emoza_header_elements_layout_7_8'
	 *
	 * @since 1.0.0
	 */
	return apply_filters( 'emoza_header_elements_layout_7_8', $elements );
}

/**
 * Mobile Header elements
 */
function emoza_mobile_header_elements() {

	$elements = array(
		'search'                   => esc_html__( 'Search', 'emoza-woocommerce' ),
		'mobile_woocommerce_icons' => esc_html__( 'Cart &amp; account icons', 'emoza-woocommerce' ),
		'button'                   => esc_html__( 'Button', 'emoza-woocommerce' ),
		'contact_info'             => esc_html__( 'Contact info', 'emoza-woocommerce' ),
	);

	/**
	 * Hook 'emoza_mobile_header_elements'
	 *
	 * @since 1.0.0
	 */
	return apply_filters( 'emoza_mobile_header_elements', $elements );
}

/**
 * Mobile Offcanvas Header elements
 */
function emoza_mobile_offcanvas_header_elements() {

	$elements = array(
		'search'                             => esc_html__( 'Search', 'emoza-woocommerce' ),
		'mobile_offcanvas_woocommerce_icons' => esc_html__( 'Cart &amp; account icons', 'emoza-woocommerce' ),
		'button'                             => esc_html__( 'Button', 'emoza-woocommerce' ),
		'contact_info'                       => esc_html__( 'Contact info', 'emoza-woocommerce' ),
	);

	/**
	 * Hook 'emoza_mobile_offcanvas_header_elements'
	 *
	 * @since 1.0.0
	 */
	return apply_filters( 'emoza_mobile_offcanvas_header_elements', $elements );
}
...
function emoza_get_default_topbar_components() {
	...
	/**
	 * Hook 'emoza_default_topbar_components'
	 *
	 * @since 1.0.0
	 */
 ...
 */
function emoza_header_transparent_choices() {

	$choices = array(
		'front-page'        => __( 'Front Page', 'emoza-woocommerce' ),
		'pages'             => __( 'Pages', 'emoza-woocommerce' ),
		'blog-archive'      => __( 'Blog Archive', 'emoza-woocommerce' ),
		'blog-posts'        => __( 'Blog Posts', 'emoza-woocommerce' ),
		'post-search'       => __( 'Posts Search Results', 'emoza-woocommerce' ),
		'404'               => __( '404 Page', 'emoza-woocommerce' ),
	);

	// Shop
	if( class_exists( 'Woocommerce' ) ) {
		$choices['shop-catalog'] = __( 'Shop Catalog', 'emoza-woocommerce' );
		$choices['shop-products'] = __( 'Shop Products', 'emoza-woocommerce' );
		$choices['shop-cart'] = __( 'Shop Cart', 'emoza-woocommerce' );
		$choices['shop-checkout'] = __( 'Shop Checkout', 'emoza-woocommerce' );
		$choices['shop-my-account'] = __( 'Shop My Account', 'emoza-woocommerce' );
		...
		if( $wishlist_enable && 'layout1' !== $wishlist_layout ) {
			$choices['shop-wishlist'] = __( 'Shop Wishlist', 'emoza-woocommerce' );
		}

	}

	/**
	 * Hook 'emoza_header_transparent_choices'
	 *
	 * @since 1.0.0
	 */
	return apply_filters( 'emoza_header_transparent_choices', $choices );
}
...
function emoza_google_fonts_url() {
	 ...
	 * This avoid issues with old Emoza users that imported demos with 
	...
	$load_locally = Emoza_Modules::is_module_active( 'local-google-fonts' );
	if( $load_locally ) {
 ...
 */
function emoza_custom_google_fonts_url() {

	$fonts_url     = '';
	$subsets       = 'latin';
	$font_families = array();
	$google_fonts  = emoza_get_google_fonts();
	...
	$body_custom_font                        = get_theme_mod( 'emoza_body_custom_font', '' );
	$body_custom_font_weight                 = get_theme_mod( 'emoza_body_custom_font_weight', '' );
	$headings_custom_font                    = get_theme_mod( 'emoza_headings_custom_font', '' );
	$headings_custom_font_weight             = get_theme_mod( 'emoza_headings_custom_font_weight', '' );
	$header_menu_custom_font                 = get_theme_mod( 'emoza_header_menu_custom_font', '' );
	$header_menu_custom_font_weight          = get_theme_mod( 'emoza_header_menu_custom_font_weight', '' );
 ...
 * Get google fonts
 */
function emoza_get_google_fonts() {
 ...
 */
function emoza_google_fonts_version() {
	$load_locally = Emoza_Modules::is_module_active( 'local-google-fonts' );
	if( $load_locally ) {
		return EMOZA_VERSION;
	}
...
function emoza_preconnect_google_fonts() {
	...
	$body_font                       = get_theme_mod( 'emoza_body_font', $defaults );
	$headings_font                   = get_theme_mod( 'emoza_headings_font', $defaults );
	$header_menu_font                = get_theme_mod( 'emoza_headings_font', $body_font );
	$button_font                     = get_theme_mod( 'button_font', $defaults );
	$button_font_style               = get_theme_mod( 'button_font_style', 'custom' );
	$loop_post_title_font            = get_theme_mod( 'loop_post_title_font', $defaults );
	$loop_post_title_font_style      = get_theme_mod( 'loop_post_title_font_style', 'heading' );
 ...
 * Get columns class
 */
function emoza_get_column_class( $number_of_columns ) {
 ...
 */
function emoza_get_social_share_url( $social ) {
	global $post;
 ...
 */
function emoza_localize_carousel_options() {
 ...
 * Emoza get image
 */
function emoza_get_image( $image_id = '', $size = 'thumbnail', $do_echo = false ) {
	if( ! $image_id ) {
		return '';
	}
	...
	if( strpos( $image_src[0], '.svg' ) !== FALSE ) {
		$output .= '<div class="emoza-image is-svg" style="mask: url('. esc_attr( $image_src[0] ) .') no-repeat center / contain; -webkit-mask: url('. esc_attr( $image_src[0] ) .') no-repeat center / contain">';
 ...
 */
function emoza_get_header_search_icon( $do_echo = false ) {
	$icon = get_theme_mod( 'search_icon', 'icon-search' );

	$output = '';
	if( $icon !== 'icon-custom' ) {
		$output .= '<i class="ws-svg-icon icon-search active">' . emoza_get_svg_icon( $icon ) . '</i>';
	} else {
		$image_id = get_theme_mod( 'search_icon_custom_image', 0 );

		/**
		 * Hook 'emoza_header_icons_image_size'
		 *
		 * @since 1.0.0
		 */
		$output .= '<i class="ws-svg-icon icon-search active">' . emoza_get_image( $image_id, apply_filters( 'emoza_header_icons_image_size', 'emoza-header-icons' ) ) . '</i>';
	}

	$output .= '<i class="ws-svg-icon icon-cancel">' . emoza_get_svg_icon( 'icon-cancel' ) . '</i>';
 ...
 * Get Header Icon
 */
function emoza_get_header_icon( $identifier = '', $do_echo = false ) {
	if( ! $identifier ) {
		return '';
	}
	...
	$output = '';
	if( $icon !== 'icon-custom' ) {
		$output .= emoza_get_svg_icon( $icon );
	} else {
		 ...
		 * @since 1.0.0
		 */
		$output .= emoza_get_image( $image_id, apply_filters( 'emoza_header_icons_image_size', 'emoza-header-icons' ) );
	}
 ...
 */
function emoza_get_header_search_form_icon( $do_echo = false ) {
	...
	if( $icon !== 'icon-custom' ) {
		...
		/**
		 * Hook 'emoza_header_search_form_icon_image_size'
		 *
		 * @since 1.0.0
		 */
		$output .= emoza_get_image( $image_id, apply_filters( 'emoza_header_search_form_icon_image_size', 'emoza-header-icons' ) );
	}
 ...
 */
function emoza_get_registered_sidebars() {
	global $wp_registered_sidebars;

	$sidebars = array(
		'' => esc_html__( 'Default', 'emoza-woocommerce' ),
	);
 ...
 */
function emoza_get_custom_fonts() {
 ...
 * 
 */
function emoza_get_permalink( $post = 0 ) {
	if ( ! is_object( $post ) ) {
		$post = get_post( $post );
	}
 ...
 */
function emoza_get_display_conditions( $maybe_rules, $default_value = true, $mod_default = '[]' ) {
	$rules  = array();
	...
	/**
	 * Hook 'emoza_display_conditions_result'
	 *
	 * @since 1.0.0
	 */
	$result = apply_filters( 'emoza_display_conditions_result', $result, $rules );
 ...
 */
function emoza_get_display_conditions_select_options( $term, $source ) {
	$options = array();
 ...
 */
function emoza_get_display_condition_value_text( $value ) {
 ...
 */
function emoza_display_conditions_script_template() {
	$settings = array();

	$settings['types'][] = array(
		'id'   => 'include',
		'text' => esc_html__( 'Include', 'emoza-woocommerce' ),
	);

	$settings['types'][] = array(
		'id'   => 'exclude',
		'text' => esc_html__( 'Exclude', 'emoza-woocommerce' ),
	);

	$settings['display'][] = array(
		'id'   => 'all',
		'text' => esc_html__( 'Entire Site', 'emoza-woocommerce' ),
	);

	$settings['display'][] = array(
		'id'      => 'basic',
		'text'    => esc_html__( 'Basic', 'emoza-woocommerce' ),
		'options' => array(
			array(
				'id'   => 'singular',
				'text' => esc_html__( 'Singulars', 'emoza-woocommerce' ),
			),
			array(
				'id'   => 'archive',
				'text' => esc_html__( 'Archives', 'emoza-woocommerce' ),
			),
		),
	);
	...
	$settings['display'][] = array(
		'id'      => 'posts',
		'text'    => esc_html__( 'Posts', 'emoza-woocommerce' ),
		'options' => array(
			array(
				'id'   => 'single-post',
				'text' => esc_html__( 'Single Post', 'emoza-woocommerce' ),
			),
			array(
				'id'   => 'post-archives',
				'text' => esc_html__( 'Post Archives', 'emoza-woocommerce' ),
			),
			array(
				'id'   => 'post-categories',
				'text' => esc_html__( 'Post Categories', 'emoza-woocommerce' ),
			),
			array(
				'id'   => 'post-tags',
				'text' => esc_html__( 'Post Tags', 'emoza-woocommerce' ),
			),
		),
	);
	...
	$settings['display'][] = array(
		'id'      => 'pages',
		'text'    => esc_html__( 'Pages', 'emoza-woocommerce' ),
		'options' => array(
			array(
				'id'   => 'single-page',
				'text' => esc_html__( 'Single Page', 'emoza-woocommerce' ),
			),
		),
	);
	...
	if ( class_exists( 'WooCommerce' ) ) {
		...
		$settings['display'][] = array(
			'id'      => 'woocommerce',
			'text'    => esc_html__( 'WooCommerce', 'emoza-woocommerce' ),
			'options' => array(
				array(
					'id'   => 'cart-page',
					'text' => esc_html__( 'Cart', 'emoza-woocommerce' ),
				),
				array(
					'id'   => 'checkout-page',
					'text' => esc_html__( 'Checkout', 'emoza-woocommerce' ),
				),
				array(
					'id'   => 'single-product',
					'text' => esc_html__( 'Single Product', 'emoza-woocommerce' ),
				),
				array(
					'id'   => 'product-archives',
					'text' => esc_html__( 'Product Archives', 'emoza-woocommerce' ),
				),
				array(
					'id'   => 'product-categories',
					'text' => esc_html__( 'Product Categories', 'emoza-woocommerce' ),
				),
				array(
					'id'   => 'product-tags',
					'text' => esc_html__( 'Product Tags', 'emoza-woocommerce' ),
				),
				array(
					'id'   => 'product-id',
					'text' => esc_html__( 'Product Name', 'emoza-woocommerce' ),
					'ajax' => true,
				),
				array(
					'id'   => 'product-category-id',
					'text' => esc_html__( 'Product Category Name', 'emoza-woocommerce' ),
					'ajax' => true,
				),
				array(
					'id'   => 'account-page',
					'text' => esc_html__( 'My Account', 'emoza-woocommerce' ),
				),
				array(
					'id'   => 'edit-account-page',
					'text' => esc_html__( 'Edit Account', 'emoza-woocommerce' ),
				),
				array(
					'id'   => 'order-received-page',
					'text' => esc_html__( 'Order Thank You', 'emoza-woocommerce' ),
				),
				array(
					'id'   => 'view-order-page',
					'text' => esc_html__( 'View Order', 'emoza-woocommerce' ),
				),
				array(
					'id'   => 'lost-password-page',
					'text' => esc_html__( 'Lost Password', 'emoza-woocommerce' ),
				),
			),
		);
	...
	$settings['display'][] = array(
		'id'      => 'specifics',
		'text'    => esc_html__( 'Specific', 'emoza-woocommerce' ),
		'options' => array(
			array(
				'id'   => 'post-id',
				'text' => esc_html__( 'Post Name', 'emoza-woocommerce' ),
				'ajax' => true,
			),
			array(
				'id'   => 'page-id',
				'text' => esc_html__( 'Page Name', 'emoza-woocommerce' ),
				'ajax' => true,
			),
			array(
				'id'   => 'category-id',
				'text' => esc_html__( 'Category Name', 'emoza-woocommerce' ),
				'ajax' => true,
			),
			array(
				'id'   => 'tag-id',
				'text' => esc_html__( 'Tag Name', 'emoza-woocommerce' ),
				'ajax' => true,
			),
			array(
				'id'   => 'author-id',
				'text' => esc_html__( 'Author Name', 'emoza-woocommerce' ),
				'ajax' => true,
			),
		),
	);
	...
	if ( ! empty( $available_post_types ) ) {
		...
		$settings['display'][] = array(
			'id'      => 'cpt',
			'text'    => esc_html__( 'Custom Post Types', 'emoza-woocommerce' ),
			'options' => array(
				array(
					'id'   => 'cpt-post-id',
					'text' => esc_html__( 'CPT: Post Name', 'emoza-woocommerce' ),
					'ajax' => true,
				),
				array(
					'id'   => 'cpt-term-id',
					'text' => esc_html__( 'CPT: Term Name', 'emoza-woocommerce' ),
					'ajax' => true,
				),
				array(
					'id'   => 'cpt-taxonomy-id',
					'text' => esc_html__( 'CPT: Taxonomy Name', 'emoza-woocommerce' ),
					'ajax' => true,
				),
			),
		);
	...
	$settings['display'][] = array(
		'id'      => 'other',
		'text'    => esc_html__( 'Other', 'emoza-woocommerce' ),
		'options' => array(
			array(
				'id'   => 'front-page',
				'text' => esc_html__( 'Front Page', 'emoza-woocommerce' ),
			),
			array(
				'id'   => 'blog',
				'text' => esc_html__( 'Blog', 'emoza-woocommerce' ),
			),
			array(
				'id'   => 'search',
				'text' => esc_html__( 'Search', 'emoza-woocommerce' ),
			),
			array(
				'id'   => '404',
				'text' => esc_html__( '404', 'emoza-woocommerce' ),
			),
			array(
				'id'   => 'author',
				'text' => esc_html__( 'Author', 'emoza-woocommerce' ),
			),
			array(
				'id'   => 'privacy-policy-page',
				'text' => esc_html__( 'Privacy Policy Page', 'emoza-woocommerce' ),
			),
		),
	);
	...
	$settings['user'][] = array(
		'id'      => 'user-auth',
		'text'    => esc_html__( 'User Auth', 'emoza-woocommerce' ),
		'options' => array(
			array(
				'id'   => 'logged-in',
				'text' => esc_html__( 'User Logged In', 'emoza-woocommerce' ),
			),
			array(
				'id'   => 'logged-out',
				'text' => esc_html__( 'User Logged Out', 'emoza-woocommerce' ),
			),
		),
	);
	...
	$settings['user'][] = array(
		'id'      => 'user-roles',
		'text'    => esc_html__( 'User Roles', 'emoza-woocommerce' ),
		'options' => $user_roles,
	);
	...
	/**
	 * Hook 'emoza_display_conditions_script_settings'
	 *
	 * @since 1.0.0
	 */
	$settings = apply_filters( 'emoza_display_conditions_script_settings', $settings );

	?>
		<script type="text/javascript">
			var emozaDCSettings = <?php echo wp_json_encode( $settings ); ?>;
		</script>
		<script type="text/template" id="tmpl-emoza-display-conditions-template">
			<div class="emoza-display-conditions-modal">
				<div class="emoza-display-conditions-modal-outer">
					<div class="emoza-display-conditions-modal-header">
						<h3>{{ data.title || data.label }}</h3>
						<i class="emoza-button-close emoza-display-conditions-modal-toggle dashicons dashicons-no-alt"></i>
					</div>
					<div class="emoza-display-conditions-modal-content">
						<ul class="emoza-display-conditions-modal-content-list">
							<li class="emoza-display-conditions-modal-content-list-item hidden">
								<div class="emoza-display-conditions-select2-type" data-type="include">
									<select name="type">
										<# _.each( emozaDCSettings.types, function( type ) { #>
											<option value="{{ type.id }}">{{ type.text }}</option>
										<# }); #>
									</select>
								</div>
								<div class="emoza-display-conditions-select2-groupped">
									<# _.each( ['display', 'user'], function( conditionGroup ) { #>
										<div class="emoza-display-conditions-select2-condition" data-condition-group="{{ conditionGroup }}">
											<select name="condition">
												<# _.each( emozaDCSettings[ conditionGroup ], function( condition ) { #>
									...
									<# }); #>
									<div class="emoza-display-conditions-select2-id hidden">
										<select name="id"></select>
									</div>
								</div>
								<div class="emoza-display-conditions-modal-remove">
									<i class="dashicons dashicons-trash"></i>
								</div>
							</li>
							<# _.each( data.values, function( value ) { #>
								<li class="emoza-display-conditions-modal-content-list-item">
									<div class="emoza-display-conditions-select2-type" data-type="{{ value.type }}">
										<select name="type">
											<# _.each( emozaDCSettings.types, function( type ) { #>
									...
									<div class="emoza-display-conditions-select2-groupped">
										<# 
											...
											_.each( emozaDCSettings, function( conditionValues, conditionKey ) {
										...
										<# if ( ! _.isEmpty( currentCondition ) ) { #>
											<div class="emoza-display-conditions-select2-condition" data-condition-group="{{ currentCondition }}">
												<select name="condition">
													<# _.each( emozaDCSettings[ currentCondition ], function( condition ) { #>
														<# if ( _.isEmpty( condition.options ) ) { #>
															<option value="{{ condition.id }}">{{ condition.text }}</option>
														<# } else { #>
															<optgroup label="{{ condition.text }}">
						...
						<div class="emoza-display-conditions-modal-content-footer">
							<a href="#" class="button emoza-display-conditions-modal-add" data-condition-group="display"><?php esc_html_e( 'Add Display Condition', 'emoza-woocommerce' ); ?></a>
							<a href="#" class="button emoza-display-conditions-modal-add" data-condition-group="user"><?php esc_html_e( 'Add User Condition', 'emoza-woocommerce' ); ?></a>
						</div>
					</div>
					<div class="emoza-display-conditions-modal-footer">
						<a href="#" class="button button-primary emoza-display-conditions-modal-save emoza-display-conditions-modal-toggle"><?php esc_html_e( 'Save Conditions', 'emoza-woocommerce' ); ?></a>
					</div>
				</div>
			</div>
		</script>
