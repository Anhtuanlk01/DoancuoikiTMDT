...
class Emoza_Merchant_Overlaping_Features_Modal {
	...
	private $disable_settings_map = array(
		...
		'quick-view' => array(
			'customizer_section' => 'emoza_section_shop_archive_product_card',
		...
		'side-cart' => array(
			'customizer_section' => 'emoza_section_shop_cart',
			'theme_mod_id'    => 'mini_cart_style',
			'theme_mod_value' => 'default',
		),
		'reasons-to-buy' => array(
			'customizer_section' => 'emoza_section_single_product_layout',
			'theme_mod_id'    => 'single_product_elements_order',
			'theme_mod_value' => array( 'emoza_single_product_reasons_to_buy' ),
		),
		'product-brand-image' => array( 
			'customizer_section' => 'emoza_section_single_product_layout',
			'theme_mod_id'    => 'single_product_elements_order',
			'theme_mod_value' => array( 'emoza_single_product_brand_image' ),
		),
		'trust-badges' => array( 
			'customizer_section' => 'emoza_section_single_product_layout',
			'theme_mod_id'    => 'single_product_elements_order',
			'theme_mod_value' => array( 'emoza_single_product_trust_badge_image' ),
		),
		'real-time-search' => array( 
			'customizer_section' => 'emoza_section_shop_search',
			'theme_mod_id'    => 'shop_search_enable_ajax',
			'theme_mod_value' => '',
		),
		'scroll-to-top-button' => array( 
			'customizer_section' => 'emoza_section_scrolltotop',
			'theme_mod_id'    => 'enable_scrolltop',
			'theme_mod_value' => '',
		),
		'size-chart' => array(
			'has_module' => true,
			'customizer_section' => 'emoza_section_single_product_size_chart',
		),
		'sticky-add-to-cart' => array(
			'has_module' => true,
			'customizer_section' => 'emoza_section_single_product_sticky_add_to_cart',
		),
		'advanced-reviews' => array(
			'has_module' => true,
			'customizer_section' => 'emoza_section_single_product_advanced_reviews',
		),
		'buy-now' => array(
			'has_module' => true,
			'customizer_section' => 'emoza_section_buy_now',
		),
		'product-video' => array(
			'has_module' => true,
		...
		'wishlist' => array(
			'has_module' => true,
			'customizer_section' => 'emoza_section_wishlist',
		),
		'product-swatches' => array(
			'has_module' => true,
			'customizer_section' => 'emoza_section_product_swatches',
		),
		'quick-social-links' => array(
			'has_module' => true,
			'module_id'  => 'quick-links',
			'customizer_section' => 'emoza_quicklinks',
		),
		'address-autocomplete' => array(
			'has_module' => true,
			'module_id'  => 'google-autocomplete',
			'customizer_section' => 'emoza_google_autocomplete_section',
		),
	);
	...
	public function enqueue_admin_css() {
		...
		wp_enqueue_style( 'emoza-admin-modal' );
		...
		$css = "
			...
			.emoza-admin-modal-content {
				border-radius: 8px;
			}

			.emoza-admin-modal-body {
				position: relative;
				padding: 40px 30px 30px 40px;
			}

			.emoza-admin-close-modal {
				position: absolute;
				top: 18px;
				right: 16px;
				transition: ease opacity 300ms;
			}
			.emoza-admin-close-modal:hover {
				opacity: 0.7;
			}
			...
			.btm-modules-modal-link-emoza {
				color: #212121;
			}
			.btm-modules-modal-link-emoza svn path {
				fill: #212121;
				transition: ease fill 300ms;
			}
			.btm-modules-modal-link-emoza:hover {
				color: #757575;
			}
			.btm-modules-modal-link-emoza:hover svg path {
				fill: #757575;
			}
		...
		";
		wp_add_inline_style( 'emoza-admin-modal', $css );
	}
	...
	public function enqueue_admin_js() {
		...
		wp_enqueue_script( 'emoza-admin-modal' );
		wp_localize_script( 'emoza-admin-modal', 'emoza_admin_modal', array( 
			'loading'     => esc_html__( 'Loading...', 'emoza-woocommerce' ),
			'redirecting' => esc_html__( 'Redirecting...', 'emoza-woocommerce' ),
		) );

		$emoza_dashboard_url = add_query_arg( array( 'page' => 'emoza-dashboard' ), admin_url( 'admin.php' ) );
		$customize_url        = admin_url( 'customize.php' );
		$js = "
			(function($) {
				
				adminEmoza.modal.triggerSelector = '.merchant-module-activate.merchant-module-deactivated-by-bp';

				$( document ).ready( function() {
					const customizerSectionsMap = " . wp_json_encode( $this->disable_settings_map ) . ";

					$( '.emoza-admin-modal' ).on( 'emoza-admin-modal-opened', function(e, modalPopup, modalTrigger) {
						...
						const customizerSectionLink = customizerSection ? '" . esc_js( $customize_url ) . "?autofocus[section]=' + customizerSection : '" . esc_js( $emoza_dashboard_url ) . "';
						...
						$( '.btm-modules-modal-link-merchant' ).off( 'click' ).on( 'click', function(e){
							...
							_this
								...
								.text( emoza_admin_modal.loading );
							...
							$.ajax({
								...
								success: function( response ) {
									if ( response.success ) {
										_this.text( emoza_admin_modal.redirecting );
		...
		";
		wp_add_inline_script( 'emoza-admin-modal', $js );
	}
	...
	public function ajax_enable_merchant_module() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'You do not have permission to perform this action', 'emoza-woocommerce' ) ) );
		}

		$module_id = isset( $_POST['module_id'] ) ? sanitize_text_field( $_POST['module_id'] ) : false;
		if ( ! $module_id ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Invalid module ID', 'emoza-woocommerce' ) ) );
		}
		...
		if ( isset( $module_settings['has_module'] ) && $module_settings['has_module'] ) {
			...
			if ( in_array( $mutable_module_id, $emoza_modules ) ) {
				$emoza_modules[ $mutable_module_id ] = false;
				update_option( 'emoza-modules', $emoza_modules );
			}
		...
		wp_send_json_success( array( 
			'message'  => esc_html__( 'Merchant module enabled', 'emoza-woocommerce' ),
	...
	public function modal_content() {
		...
		<div class="emoza-admin-modal">
			<div class="emoza-admin-modal-content">
				<div class="emoza-admin-modal-body btm-modules-modal">
					<a href="#" class="emoza-admin-close-modal" title="<?php echo esc_attr__( 'Close', 'emoza-woocommerce' ); ?>">
					...
					<h2 class="btm-modules-modal-title">
						...
						<?php echo esc_html__( 'This module conflicts with Emoza!', 'emoza-woocommerce' ); ?>
					</h2>
					<div class="btm-modules-modal-actions">
						<div>
							<h3><?php echo esc_html__( 'Continue with Emoza', 'emoza-woocommerce' ); ?></h3>
							<p><?php echo esc_html__( 'To maintain the Emoza module as your primary option over the Merchant module, please click the button below to access the Emoza module settings.', 'emoza-woocommerce' ); ?></p>
							<a href="#" class="btm-modules-modal-link btm-modules-modal-link-emoza" title="<?php echo esc_attr__( 'Go to Emoza settings', 'emoza-woocommerce' ); ?>" target="_blank">
								<?php echo esc_html__( 'Go to Emoza settings', 'emoza-woocommerce' ); ?>
						...
						<div>
							<h3><?php echo esc_html__( 'Switch to Merchant', 'emoza-woocommerce' ); ?></h3>
							<p><?php echo esc_html__( 'To disable the Emoza module and activate the corresponding Merchant module, please click the button below. This action will disable the Emoza module.', 'emoza-woocommerce' ); ?></p>
							<a href="#" class="btm-modules-modal-link btm-modules-modal-link-merchant" title="<?php echo esc_attr__( 'Enable Merchant module', 'emoza-woocommerce' ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'btm-enable-merchant-module' ) ); ?>">
								<?php echo esc_html__( 'Enable Merchant module', 'emoza-woocommerce' ); ?>
							</a>
						</div>
					</div>
				</div>
...
new Emoza_Merchant_Overlaping_Features_Modal();