...
class Emoza_Merchant_Single_Product_Elements {
	...
	public static $modules_data = array(
		'payment-logos' => array(
			...
			'callback'         => 'emoza_merchant_payment_logos',
		),
		'trust-badges' => array(
			'class'            => 'Merchant_Trust_Badges',
			'callback'         => 'emoza_merchant_trust_badges',
		),
		'product-bundles' => array(
			'class'             => 'Merchant_Product_Bundles',
			'callback'          => 'emoza_merchant_product_bundles',
		),
		'buy-x-get-y' => array(
			'class'             => 'Merchant_Buy_X_Get_Y',
			'callback'          => 'emoza_merchant_buy_x_get_y',
		),
		'volume-discounts' => array(
			'class'             => 'Merchant_Volume_Discounts',
			'callback'          => 'emoza_merchant_volume_discounts',
		),
		'wait-list' => array(
			'class'             => 'Merchant_Wait_List',
			'callback'          => 'emoza_merchant_wait_list',
		),
		'stock-scarcity' => array(
			'class'             => 'Merchant_Stock_Scarcity',
			'callback'          => 'emoza_merchant_stock_scarcity',
		),
		'reasons-to-buy' => array(
			'class'             => 'Merchant_Reasons_To_Buy',
			'callback'          => 'emoza_merchant_reasons_to_buy',
		),
		'product-brand-image' => array(
			'class'             => 'Merchant_Product_Brand_Image',
			'callback'          => 'emoza_merchant_product_brand_image',
		),
		'size-chart' => array(
			'class'             => 'Merchant_Size_Chart',
			'callback'          => 'emoza_merchant_size_chart',
		),
	);
	...
	public static function get_module_title( $module_id ) {
		$titles = array(
			'payment-logos'       => esc_html__( 'Merchant: Payment Logos', 'emoza-woocommerce' ),
			'trust-badges'        => esc_html__( 'Merchant: Trust Badges', 'emoza-woocommerce' ),
			'product-bundles'     => esc_html__( 'Merchant: Product Bundles', 'emoza-woocommerce' ),
			'buy-x-get-y'         => esc_html__( 'Merchant: Buy X Get Y', 'emoza-woocommerce' ),
			'volume-discounts'    => esc_html__( 'Merchant: Bulk Discounts', 'emoza-woocommerce' ),
			'wait-list'           => esc_html__( 'Merchant: Waitlist', 'emoza-woocommerce' ),
			'stock-scarcity'      => esc_html__( 'Merchant: Stock Scarcity', 'emoza-woocommerce' ),
			'reasons-to-buy'      => esc_html__( 'Merchant: Reasons To Buy', 'emoza-woocommerce' ),
			'product-brand-image' => esc_html__( 'Merchant: Product Brand Image', 'emoza-woocommerce' ),
			'size-chart'          => esc_html__( 'Merchant: Size Chart', 'emoza-woocommerce' ),
		);
	...
	public static function get_module_admin_field_descriptions() {
		$customizer_section_url = admin_url( 'customize.php?autofocus[section]=emoza_section_single_product_layout' );
		$default_desc           = sprintf(
			/* Translators: 1. Customizer link. 2. Link text. */
			__( 'The display is being controlled by Emoza under <a href="%1$s" target="_blank">%2$s</a>', 'emoza-woocommerce' ),
			$customizer_section_url,
			__( 'Customizer > WooCommerce > Single Product > Layout (Elements)', 'emoza-woocommerce' )
		);
	...
	public function __construct() {
		...
		/**
		 * Hook 'emoza_merchant_modules_single_product_integration'
		 * Filters whether to integrate Merchant modules with Emoza single product elements.
		 * 
		 * @since 1.1.0
		 */
		if ( ! apply_filters( 'emoza_merchant_modules_single_product_integration', true ) ) {
			return;
		}

		if ( get_option( 'emoza_merchant_modules_single_product_integration', 'yes' ) === 'no' ) {
			return;
		}
		...
		add_filter( 'emoza_default_single_product_components', array( $this, 'customizer_components_value' ) );
		add_filter( 'emoza_single_product_elements', array( $this, 'customizer_elements' ) );

		// Merchant won't render the modules output if the shortcode option is off. So, we need to force it to be on.
		// This needs to be done via 'emoza_merchant_before_render_shortcode' because we don't want to force the shortcode functionality enable to the modules
		// when some page builder such as emoza templates builder, elementor, wpbakery, beaver, etc, are in use. 
		add_action( 'emoza_before_render_single_product_elements', array( $this, 'turn_on_merchant_modules_shortcode_functionality' ) );
	}
	...
	public function add_internal_style() {
		...
		if ( ! empty( $page ) && 'merchant' === $page && isset( $_GET['module'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$css = "
				.emoza-merchant-module-field-disabled .merchant-module-page-setting-field-inner {
					pointer-events: none;
					opacity: 0.5;
				}

				.emoza-merchant-module-field-disabled .merchant-module-page-setting-field-desc {
					font-size: 14px;
					line-height: 1.6;
					color: #1d2327;
					border-radius: 4px;
	...
	public function add_module_to_customizer_single_product_elements( $module_id ) {
		...
		$current_mod_value = get_theme_mod( 'single_product_elements_order', emoza_get_default_single_product_components() );
	...
	public function admin_field_wrapper_class( $classes, $settings, $value, $module_id ) {
		...
		$classes[] = 'emoza-merchant-module-field-disabled';
		return $classes;
	}
...
new Emoza_Merchant_Single_Product_Elements();
