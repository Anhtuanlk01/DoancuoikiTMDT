...
class Emoza_Upsell_Message extends WP_Customize_Control {
		
	/**
	 * The type of control being rendered
	 */
	public $type          = 'emoza-upsell-features';
	public $title         = '';
	public $sub_title     = '';
	...
	public $button_text   = '';
	public $button_link   = 'https://emoza.org?utm_source=theme_customizer_deep&utm_medium=button&utm_campaign=Emoza';
	public $all_features_link = 'https://emoza.org?utm_source=theme_customizer_deep&utm_medium=button&utm_campaign=Emoza#see-all-features';
	...
	public function __construct( $manager, $id, $args = array(), $options = array() ) {
		...
		$this->button_text = esc_html__( 'Upgrade to Emoza Pro', 'emoza-woocommerce' );

		if ( empty( $this->sub_title ) ) {
			$this->sub_title = esc_html__( 'Unlock all features:', 'emoza-woocommerce' );
		}

		if ( empty( $this->features_list_last_item_text ) ) {
			$this->features_list_last_item_text = array(
				'text_before_link' => '...',
				'link_text'        => esc_html__( 'and many other premium features', 'emoza-woocommerce' ),
			);
		}
	}
	...
	public function render_content() {
		...
		?>
		<div class="emoza-upsell-feature-wrapper">
			<?php if( $this->display_thumb ) : ?>
				<div class="emoza-upsell-feature-thumb">
					<img src="<?php echo esc_url( get_template_directory_uri() . '/assets/img/admin/customizer-upsell-thumbnail.png' ); ?>" alt="<?php esc_attr_e( 'Emoza Pro', 'emoza-woocommerce' ); ?>" />
				</div>
			<?php endif; ?>
			...
			<?php if( ! empty( $this->features_list ) ) : ?>
				<ul class="emoza-upsell-features-list">
					...
					<li>
						...
						<?php 
						/* translators: %1$s: link to Emoza Pro, %2$s: link text */
						printf(
			...
			<?php endif; ?>
			<a href="<?php echo esc_url( $this->button_link ); ?>" role="button" class="emoza-upsell-button" target="_blank">
