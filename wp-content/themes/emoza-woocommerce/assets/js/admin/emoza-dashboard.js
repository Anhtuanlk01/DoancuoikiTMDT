  ...
  $(document).ready(function () {
        ...
        $.post(window.emoza_dashboard.ajax_url, {
          ...
          if (response.success) {
            if (activate) {
              ...
              $this.parent().find('.emoza-dashboard-customize-link').removeClass('bt-d-none');
              if (hasRedirect) {
              ...
              $this.parent().find('.emoza-dashboard-customize-link').addClass('bt-d-none');
            }
          }
        });
      });
    ...
    if ($template_save.length) {
      $template_save.on('click', function (e) {
        e.preventDefault();
        $(this).html('<i class="dashicons dashicons-update-alt"></i>' + window.emoza_dashboard.i18n.saving);
        var data = [];
        ...
        $.post(window.emoza_dashboard.ajax_url, {
          action: 'emoza_template_builder_data',
          data: data,
          nonce: window.emoza_dashboard.nonce
        }, function (response) {
          if (response.success) {
            $template_save.html(window.emoza_dashboard.i18n.saved);
            $('#save-templates').addClass('saved');
            setTimeout(function () {
              $template_save.html(window.emoza_dashboard.i18n.save);
              $template_save.blur();
            }, 2000);
          }
        });
    ...
    if ($add_template.length) {
      $('#add-new-template').on('click', function () {
        ...
        $template_item.attr('data-id', 'emoza-template-' + Math.random().toString(36).slice(2));
    ...
    $(document).on('click', '.template-options .duplicate-template', function (e) {
      ...
      $template_item.attr('data-id', 'emoza-template-' + Math.random().toString(36).slice(2)); //replace id
    ...
    $(document).on('click', '.page-builder-wrapper .create-new', function () {
      ...
      var $modalContainer = $('.emoza-elementor-iframe-wrapper');
      var editorIframe = $modalContainer.find('.emoza-elementor-iframe');
      ...
      $.post(window.emoza_dashboard.ajax_url, {
        ...
        if (response.success) {
          ...
          if (page_builder == 'elementor') {
            ...
            $(document).on('click', '.emoza-editor-close', function () {
              $modalContainer.hide();
    ...
    $(document).on('click', '.part-options .edit-part', function () {
      ...
      $(this).html('<i class="dashicons dashicons-update-alt"></i>' + window.emoza_dashboard.i18n.loading);
      $.post(window.emoza_dashboard.ajax_url, {
    ...
    function save_notice() {
      //return if free
      var $container = $('.emoza-dashboard-container');
      ...
      $(window).on('beforeunload', function () {
        if (!$('#save-templates').hasClass('saved')) {
          return window.emoza_dashboard.i18n.unsaved_changes;
        }
      });
    }
