 ...
 */
emoza.helpers = {
  emozaDomReady: function emozaDomReady(fn) {
...
emoza.navigation = {
  init: function init() {
    var self = this,
      ...
      offCanvas = document.getElementsByClassName('emoza-offcanvas-menu')[0],
      ...
      try {
        for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
          ...
          anchor.addEventListener('click', function (e) {
            if (e.target.hash && document.querySelector(e.target.hash) !== null && !e.target.classList.contains('emoza-tabs-nav-link')) {
    ...
    document.addEventListener('click', function (e) {
      if (e.target.closest('.emoza-offcanvas-menu') === null && !e.target.classList.contains('menu-toggle') && e.target.closest('.menu-toggle') === null) {
        button.classList.remove('open');
        offCanvas.classList.remove('toggled');
        document.body.classList.remove('mobile-menu-visible');
      }
  ...
  initHoverClass: function initHoverClass() {
    var self = this;
    if (typeof emoza.settings !== 'undefined' && 'no' === emoza.settings.misc.dropdowns_hover_delay) {
      return false;
    }
  ...
  addHoverClassToDropdownItems: function addHoverClassToDropdownItems() {
    var dropdownLis = document.querySelectorAll('.emoza-dropdown-li');
    ...
    try {
      var _loop = function _loop() {
        ...
        var subDropdownLis = li.querySelectorAll('.emoza-dropdown-ul > .emoza-dropdown-li');
  ...
  */
  mobileAccordionNavigation: function mobileAccordionNavigation() {
    var navs = document.querySelectorAll('.emoza-dropdown-mobile-accordion');
    if (!navs.length) {
      return false;
    }
  ...
  */
  checkMenuReverse: function checkMenuReverse() {
    var items = document.querySelectorAll('.header-login-register, .top-bar-login-register, .emoza-dropdown .menu li');
  ...
  menuReverseEventHandler: function menuReverseEventHandler() {
    ...
    if (is_rtl) {
      ...
      if (emoza.helpers.isInHorizontalViewport(submenu) == false && !submenu.closest('.menu-item').classList.contains('emoza-mega-menu')) {
        submenu.classList.add('sub-menu-reverse');
      } else {
        submenu.classList.remove('sub-menu-reverse');
      }
    ...
    if (emoza.helpers.isInVerticalViewport(submenu) == false && !submenu.closest('.menu-item').classList.contains('emoza-mega-menu')) {
 ...
 */
emoza.desktopOffCanvasToggleNav = {
  init: function init() {
    var siteNavigation = document.getElementById('site-navigation'),
      offCanvas = document.getElementsByClassName('emoza-desktop-offcanvas-menu')[0];
...
emoza.desktopOffcanvasNav = {
  init: function init() {
    var buttons = document.querySelectorAll('.desktop-menu-toggle'),
      ...
      offcanvas = document.getElementsByClassName('emoza-desktop-offcanvas')[0];
    if (!buttons.length) {
      return false;
    }
    for (var i = 0; i < buttons.length; i++) {
      buttons[i].addEventListener('click', function (e) {
        e.preventDefault();
        if (offcanvas.classList.contains('emoza-desktop-offcanvas-show')) {
          offcanvas.classList.remove('emoza-desktop-offcanvas-show');
        } else {
          offcanvas.classList.add('emoza-desktop-offcanvas-show');
        }
      });
    }
    closeButton.addEventListener('click', function (e) {
    ...
    document.addEventListener('click', function (e) {
      if (e.target.closest('.emoza-desktop-offcanvas-menu') === null && offcanvas.querySelector('.emoza-mega-menu .sub-menu.toggled') !== null) {
        offcanvas.querySelector('.emoza-mega-menu .sub-menu.toggled').classList.remove('toggled');
      }
    });
  }
};
...
emoza.headerSearch = {
  init: function init() {
      ...
      try {
        for (_iterator12.s(); !(_step12 = _iterator12.n()).done;) {
          ...
          buttonEl.addEventListener('click', function (e) {
            ...
            if (e.target.closest('.emoza-offcanvas-menu') !== null) {
              e.target.closest('.emoza-offcanvas-menu').classList.remove('toggled');
            }
          });
        }
      } catch (err) {
    ...
    var desktop_offcanvas = document.getElementsByClassName('header-desktop-offcanvas-layout2')[0] !== null ? document.getElementsByClassName('emoza-desktop-offcanvas')[0] : false;
...
emoza.stickyHeader = {
  init: function init() {
    ...
    if (topOffset > 10) {
      ...
      window.dispatchEvent(new Event('emoza.sticky.header.activated'));
    }
    ...
    if (document.body.classList.contains('emoza-site-layout-padded')) {
      header_offset_y = header_offset_y - parseInt(getComputedStyle(document.body).getPropertyValue('--emoza_padded_spacing'));
    }
    if (sticky.classList.contains('sticky-scrolltop') || document.querySelector('.bhfb.sticky-scrolltop') !== null) {
      ...
      window.addEventListener('scroll', function () {
        ...
        if (is_sticky) {
          ...
          window.dispatchEvent(new Event('emoza.sticky.header.deactivated'));
        } else {
          sticky.classList.add('is-sticky');
          body.classList.add('sticky-header-active');
  ...
  isHBStickyActive: function isHBStickyActive(effect) {
    ...
    if (bhfb.classList.contains('sticky-row-main-header-row')) {
      ...
      if (has_admin_bar) {
        ...
        if (!above_header_row.classList.contains('bt-d-none') && document.body.classList.contains('emoza-site-layout-padded')) {
          convertToPositive = false;
        }
      }
      ...
      // Padded Layout
      if (document.body.classList.contains('emoza-site-layout-padded')) {
        topVal = topVal - parseFloat(getComputedStyle(document.body).getPropertyValue('--emoza_padded_spacing'));
      }
  ...
  stickyChangeLogo: function stickyChangeLogo() {
    ...
    if (window.matchMedia('screen and (min-width: 1024px)').matches) {
      if (typeof emoza_sticky_header_logo !== 'undefined') {
        ...
        window.addEventListener('emoza.sticky.header.activated', function () {
          if (sticky_flag) {
            return false;
          }
          logo.setAttribute('src', emoza_sticky_header_logo[0]);
          logo.setAttribute('style', 'max-height: ' + initialHeight + 'px;');
          if (typeof emoza_sticky_header_logo['srcset'] !== 'undefined') {
            logo.setAttribute('srcset', emoza_sticky_header_logo['srcset']);
          }
          if (typeof emoza_sticky_header_logo['sizes'] !== 'undefined') {
            logo.setAttribute('sizes', emoza_sticky_header_logo['sizes']);
          }
          sticky_flag = true;
        });
        window.addEventListener('emoza.sticky.header.deactivated', function () {
          if (!sticky_flag) {
            return false;
          }
...
/**
 * Emoza scroll direction
 */
emoza.scrollDirection = {
  init: function init() {
    var elements = document.querySelectorAll('.emoza-single-sticky-add-to-cart-wrapper.hide-when-scroll'),
      body = document.getElementsByTagName('body')[0];
    ...
    window.addEventListener('scroll', function () {
      ...
      if (scroll > lastScrollTop) {
        body.classList.remove('emoza-scrolling-up');
        body.classList.add('emoza-scrolling-down');
      } else {
        body.classList.remove('emoza-scrolling-down');
        body.classList.add('emoza-scrolling-up');
      }
      lastScrollTop = scroll <= 0 ? 0 : scroll;
    }, false);
  }
...
/**
 * Emoza custom add to cart button
 * 
 */
emoza.customAddToCartButton = {
  init: function init() {
    var button = document.querySelectorAll('.emoza-custom-addtocart');
    if (!button.length) {
      return false;
    }
      ...
      button[i].addEventListener('click', function (e) {
        ...
        ajax.onload = function () {
          if (this.status >= 200 && this.status < 400) {
            ...
            document.body.dispatchEvent(new Event('emoza.custom_added_to_cart'));
          }
        };
        ajax.send('action=emoza_custom_addtocart&product_id=' + productId + '&nonce=' + nonce);
      });
    }
  }
};
...
emoza.backToTop = {
  ...
  safariDoubleClickFix: function safariDoubleClickFix() {
    var links = document.querySelectorAll('.product-gallery-summary .emoza-single-addtocart-wrapper .button, .single-product .content-wrapper a, .single-product .footer-widgets a, .single-product .site-footer a');
    if (!links.length) {
      return false;
    }
...
emoza.qtyButton = {
  ...
  events: function events(type) {
    var self = this;
    var qty = document.querySelectorAll('.emoza-quantity-minus');
    if (qty.length < 1) {
      return false;
    }
    for (var i = 0; i < qty.length; i++) {
      ...
      var qtyInput = wrapper.querySelector('.qty'),
        plus = wrapper.querySelector('.emoza-quantity-plus'),
        minus = wrapper.querySelector('.emoza-quantity-minus'),
  ...
  updateAddToCartQuantity: function updateAddToCartQuantity(qtyItem, qtyValue) {
    ...
    if (miniCartItem) {
      ...
      jQuery.post({
        url: emoza.ajaxurl,
        data: {
          action: 'emoza_update_mini_cart_quantity',
  ...
  updateBuyNowButtonQuantity: function updateBuyNowButtonQuantity(qtyItem, qtyValue) {
    var is_checkout_quantity = qtyItem.parentNode.parentNode.classList.contains('emoza-sc-product-name') || qtyItem.parentNode.parentNode.classList.contains('product-name') ? true : false,
    ...
    var productSelector = qtyItem.closest('.product') ? '.product' : '.wc-block-grid__product',
      product = qtyItem.closest(productSelector),
      qtyInput = qtyItem.parentNode.querySelector('.qty'),
      buyNowButton = product ? product.querySelector('.emoza-buy-now-button') : null;
...
emoza.carousel = {
  ...
  build: function build() {
    if (document.querySelector('.emoza-carousel') === null && document.querySelector('.has-cross-sells-carousel') === null && document.querySelector('.emoza-woocommerce-mini-cart__cross-sell') === null) {
      return false;
    }
    var carouselEls = document.querySelectorAll('.emoza-carousel, #masthead .cross-sells, .emoza-side-mini-cart .cross-sells, .cart-collaterals .cross-sells');
    var _iterator16 = _createForOfIteratorHelper(carouselEls),
      _step16;
    try {
      for (_iterator16.s(); !(_step16 = _iterator16.n()).done;) {
        var carouselEl = _step16.value;
        if (carouselEl.querySelector('.emoza-carousel-stage') === null) {
          carouselEl.querySelector('.products').classList.add('emoza-carousel-stage');
        }
        if (carouselEl.getAttribute('data-initialized') !== 'true') {
          ...
          var wrapper = document.createElement('div'),
            stage = carouselEl.querySelector('.emoza-carousel-stage');
          wrapper.className = 'emoza-carousel-wrapper';
          ...
          // Margin
          var margin = 30;
          if (typeof emoza_carousel !== 'undefined') {
            margin = parseInt(emoza_carousel.margin_desktop);
          } else if (carouselEl.closest('.emoza-woocommerce-mini-cart__cross-sell') !== null) {
            margin = 15;
          }
          var itemsQty = carouselEl.querySelectorAll('li.product').length;
          if (itemsQty <= parseInt(perPage)) {
            carouselEl.classList.add('emoza-carousel-not-initialized');
            return false;
          }
          ...
          var carousel = new Siema({
            ...
            onInit: function onInit() {
              window.dispatchEvent(new Event('emoza.carousel.initialized'));
            }
          });
        }
      }
 ...
 */
emoza.copyLinkToClipboard = {
  init: function init(event, el) {
    event.preventDefault();
    navigator.clipboard.writeText(window.location.href);
    el.classList.add('copied');
    el.setAttribute('data-emoza-tooltip', emoza.i18n.emoza_sharebox_copy_link_copied);
    setTimeout(function () {
      el.setAttribute('data-emoza-tooltip', emoza.i18n.emoza_sharebox_copy_link);
...
emoza.toggleClass = {
  init: function init(event, el, triggerEvent) {
    event.preventDefault();
    event.stopPropagation();
    var selector = document.querySelector(el.getAttribute('data-emoza-selector')),
      removeClass = el.getAttribute('data-emoza-toggle-class-remove'),
      classname = el.getAttribute('data-emoza-toggle-class'),
...
emoza.collapse = {
  init: function init() {
    var elements = document.querySelectorAll('[data-emoza-collapse]');
    if (!elements.length) {
      return false;
    }
    var _this = this;
    var _loop2 = function _loop2(i) {
      var opts = elements[i].getAttribute('data-emoza-collapse'),
      ...
      elements[i].addEventListener('click', function (e) {
        e.preventDefault();
        this.dispatchEvent(new Event('emoza.collapse.before.expand'));
        if (!elements[i].classList.contains('active')) {
          _this.expand(elements[i], options);
        } else {
          _this.collapse(elements[i], options);
        }
        this.dispatchEvent(new Event('emoza.collapse.after.collapse'));
      });
      if (options.options.oneAtTime) {
        elements[i].addEventListener('emoza.collapse.before.expand', function () {
          var emoza_collapse = document.querySelectorAll(options.options.oneAtTimeParentSelector + ' [data-emoza-collapse]');
          for (var u = 0; u < emoza_collapse.length; u++) {
            _this.collapseAll(emoza_collapse[u], options);
          }
        });
      }
    };
  ...
  expand: function expand(el, options, first_load) {
    ...
    var targetSelectorId = options.id,
      ...
      targetContent = target.querySelector('.emoza-collapse__content');
  ...
  expandTransitionEnd: function expandTransitionEnd(el) {
    ...
    el.dispatchEvent(new Event('emoza.collapse.expanded'));
  },
  collapse: function collapse(el, options, a) {
    ...
    el.dispatchEvent(new Event('emoza.collapse.collapsed'));
  },
...
emoza.tabsNav = {
  init: function init() {
    var tabsNav = document.querySelectorAll('.emoza-tabs-nav');
    if (!tabsNav.length) {
      return false;
    }
    this.events();
  ...
  events: function events() {
    var tabsNavItems = document.querySelectorAll('.emoza-tabs-nav-item');
    var _iterator17 = _createForOfIteratorHelper(tabsNavItems),
      _step17;
    try {
      for (_iterator17.s(); !(_step17 = _iterator17.n()).done;) {
        var tabItem = _step17.value;
        var hasClickOnMouseOver = tabItem.closest('.emoza-tabs-nav').classList.contains('emoza-tabs-nav-click-on-mouseover');
        ...
        tabItem.addEventListener('click', function (e) {
          e.preventDefault();
          var _this = this,
            tabId = this.querySelector('.emoza-tabs-nav-link').getAttribute('href'),
            previousTabId = this.closest('.emoza-tabs-nav').querySelector('.emoza-tabs-nav-item.is-active .emoza-tabs-nav-link').getAttribute('href'),
            previousActiveTab = document.querySelector(previousTabId),
            tabContentTo = document.querySelector(tabId),
            activeTabsCount = tabContentTo.parentNode.querySelectorAll('.emoza-tab-content.is-active').length;
          ...
          tabsNavItems = this.closest('.emoza-tabs-nav').querySelectorAll('.emoza-tabs-nav-item');
          var _iterator18 = _createForOfIteratorHelper(tabsNavItems),
            _step18;
          try {
            for (_iterator18.s(); !(_step18 = _iterator18.n()).done;) {
              var _tabItem = _step18.value;
              var _tabContentTo = document.querySelector(_tabItem.querySelector('.emoza-tabs-nav-link').getAttribute('href'));
    ...
    if (typeof jQuery === 'function') {
      (function ($) {
        ...
        if (is_checkout_page === null) {
          ...
          $(window).trigger('emoza.wcexpresspaybtns.appended');
        }
      })(jQuery);
    }
  },
  singleProduct: function singleProduct() {
    ...
    if (typeof jQuery === 'function') {
      (function ($) {
        ...
        if ($('.emoza-in-cart-flag').length) {
          var $form = $('body.single-product .entry-summary .variations_form, .emoza-tb-sp-add-to-cart'),
            addToCartButton = $form.find('.single_add_to_cart_button'),
            defaultAddToCartButtonText = addToCartButton.text();
          $form.each(function () {
            $(this).on('found_variation', function (event, variation) {
              var variationInCart = emozaInCartFlag.variations_in_cart.filter(function (a) {
...
emoza.helpers.emozaDomReady(function () {
  emoza.navigation.init();
  emoza.desktopOffcanvasNav.init();
  emoza.desktopOffCanvasToggleNav.init();
  emoza.headerSearch.init();
  emoza.customAddToCartButton.init();
  emoza.stickyHeader.init();
  emoza.scrollDirection.init();
